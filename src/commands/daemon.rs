...
pub async fn start_daemon() {
    ...
    let listener = TcpListener::bind("0.0.0.0:49242").unwrap_or_else(|e| {
        LOGGER.panic_crash(format!("Failed to bind to port 49242: {e}"));
    });
...
pub fn handle_transfer_request(mut stream: TcpStream) -> Result<(), Box<dyn std::error::Error>> {
    ...
    LOGGER.info(format!("Transfer request from {sender_name} for file '{file_name}'"));
